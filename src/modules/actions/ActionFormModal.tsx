...
export function ActionFormModal({ project, action, defaults, onClose }: Props) {
  ...
  const submit = () => {
    ...
    if (action) {
      updateAction(action.id, input);
    } else {
      addAction(project.id, input);
    }
    onClose();
  };
  ...
  if (project.members.length === 0) {
    return (
      <Modal title="Nouvelle action" onClose={onClose}>
        <p className="muted">
          Ajoutez d'abord des membres à l'équipe (module RACI) : une action doit obligatoirement
          être assignée à un Responsible issu du projet.
        </p>
      </Modal>
    );
  }
        ...
        <div className="field">
          ...
          {!dueDate && (
            <span className="form-warning">
              Fortement recommandée : sans échéance, l'action n'apparaît pas dans le calendrier.
            </span>
          )}
        </div>
      </div>
